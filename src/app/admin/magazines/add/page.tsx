...
export default function AdminAddMagazine() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!res.ok) throw new Error('Dergi eklenemedi')
  ...
  return (
    <div className="max-w-2xl bg-surface p-8 border border-border rounded-sm">
      <h2 className="text-3xl font-display font-medium text-foreground mb-8">Yeni Dergi Ekle</h2>
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-6 text-foreground">
        ...
        <div>
          ...
          <input required name="excerpt" className="w-full bg-bg border border-border p-3 rounded-sm focus:border-primary focus:outline-none transition-colors" />
        </div>

        <div>
          <label className="block text-xs uppercase tracking-[0.1em] text-text-secondary mb-2">Görsel URL</label>
          <input required name="imageUrl" className="w-full bg-bg border border-border p-3 rounded-sm focus:border-primary focus:outline-none transition-colors" />
        </div>
        
        <div>
          <label className="block text-xs uppercase tracking-[0.1em] text-text-secondary mb-2">İçerik</label>
          <textarea required name="content" rows={6} className="w-full bg-bg border border-border p-3 rounded-sm focus:border-primary focus:outline-none transition-colors" />
        </div>

        <button type="submit" disabled={loading} className="btn-primary mt-4 disabled:opacity-50">
          {loading ? 'Ekleniyor...' : 'Dergi Ekle'}
        </button>
      </form>
    </div>
  )
